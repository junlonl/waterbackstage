...
function setComplainMessage(json, act) {
    if (act == "accept") { $(".selfAccept-btn").show(); $(".accept-btn,.approval-btn,.accept-flow-btn").hide(); }
    else if (act == "finish") { $(".apply-message").show(); $(".accept-btn,.selfAccept-btn,.accept-flow-btn").hide(); }
    ...
    if (json.indexOf("RequestSuccessful") >= 0) {
        ...
        json = eval('(' + json + ')');
        $(".content-item-time").html(json.complainDate);
        ...
        if(act=="finish"){
        	...
        	$.ajax({
        		url:'../weixinRiverMaster/getFinishApplyInfo',
                type: 'GET',
                dataType: 'text',
                data: { complainId: complainId },
                success: function (jsons) {
                	if (jsons.indexOf("RequestSuccessful") >= 0) {
                        jsons = jsons.replace("RequestSuccessful", "");
                        if(jsons=="Null"){return;}
                        jsons = eval('(' + jsons + ')');
                        $(".apply-item-time").html(jsons.createTime);
                        $(".apply-message-head-name").html(jsons.title);
                        $(".apply-message-head-title").html(jsons.content);
                        $("#hide-complainId").val(jsons.questionId)
                        var imgCount = jsons.photoList.listCount;
                        var imglistH = (250 * Math.ceil(imgCount / 3) + 20);
                        $(".apply-message-imglist").css("height", imglistH + "px");
                        
                        var html1 = ""
                        if (jsons.photoList.listCount != 0) {
                            var size = jsons.photoList.listCount;
                            for (var i = 0; i < size; i++) {
                                var obj = jsons.photoList.list[i];
                                html1+='<img src="../weixinRiverMaster/getLittlePicById?id=' + obj.locationId + '" class="apply-message-img" />';
                            }
                        }
                        $(".apply-message-imglist").html(html1);
                	}
                }
            });
   ...
   $(".complainlist").empty();
    if (json.indexOf("RequestSuccessful") >= 0) {
        ...
        $.each(json.complainList, function (i, item) {
            ...
            h += "        <div class=\"content-item-content item-content_" + item.questionId + "\" " + clickFn + " "+iphoneFont+">" + item.questioncontent + "</div>"
...
function showSelfAccept() {
    ...
    $(".selfAccept-op-list").append("<div class=\"accept-op\" onclick=\"goComplain()\">申请完结</div>");
...
function showAccept() {
    ...
    $(".accept-op-list").append("<div class=\"accept-op\" onclick=\"saveNextDepartment(this,'next')\">跟进处理</div>");
    $(".accept-op-list").append("<div class=\"accept-op\" onclick=\"replyNo(this)\">申请完结</div>");
...
function Send() {
	...
	var content=$("#id-complain-content").val();
	var complainId=$("#hide-complainId").val();	
	...
	$.ajax({
		url:'../weixinRiverMaster/saveFinishApply',
		type:'post',
		dataType:'text',
		data:{'complainId':complainId,
			'content':content,
